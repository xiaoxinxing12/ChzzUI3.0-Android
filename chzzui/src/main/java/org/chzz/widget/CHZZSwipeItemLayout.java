 ...
 */
public class CHZZSwipeItemLayout extends FrameLayout {
    private static final String TAG = CHZZSwipeItemLayout.class.getSimpleName();
    ...
    // 手动拖动打开和关闭代理
    private CHZZSwipeItemLayoutDelegate mDelegate;
    ...
    public CHZZSwipeItemLayout(Context context, AttributeSet attrs) {
        this(context, attrs, 0);
    }

    public CHZZSwipeItemLayout(Context context, AttributeSet attrs, int defStyleAttr) {
    ...
    private void initAttrs(Context context, AttributeSet attrs) {
        final TypedArray typedArray = context.obtainStyledAttributes(attrs, R.styleable.CHZZSwipeItemLayout);
    ...
    private void initAttr(int attr, TypedArray typedArray) {
        if (attr == R.styleable.CHZZSwipeItemLayout_chzz_sil_swipeDirection) {
            // 默认向左滑动
        ...
        } else if (attr == R.styleable.CHZZSwipeItemLayout_chzz_sil_bottomMode) {
            // 默认是拉出
        ...
        } else if (attr == R.styleable.CHZZSwipeItemLayout_chzz_sil_springDistance) {
            // 弹簧距离，不能小于0，默认值为0
            mSpringDistance = typedArray.getDimensionPixelSize(attr, mSpringDistance);
            if (mSpringDistance < 0) {
                throw new IllegalStateException("CHZZ_sil_springDistance不能小于0");
            }
        } else if (attr == R.styleable.CHZZSwipeItemLayout_chzz_sil_swipeAble) {
    ...
    public void setDelegate(CHZZSwipeItemLayoutDelegate delegate) {
        mDelegate = delegate;
    }
    ...
    protected void onFinishInflate() {
        ...
        if (getChildCount() != 2) {
            throw new IllegalStateException(CHZZSwipeItemLayout.class.getSimpleName() + "必须有且只有两个子控件");
        }
    ...
    private void performAdapterViewItemClick() {
        ...
        if (t instanceof AdapterView) {
            ...
            int p = view.getPositionForView(CHZZSwipeItemLayout.this);
    ...
    private boolean performAdapterViewItemLongClick() {
        ...
        if (t instanceof AdapterView) {
            ...
            try {
                ...
                handled = (boolean) m.invoke(view, CHZZSwipeItemLayout.this, p, vId);

            } catch (Exception e) {
                e.printStackTrace();

                if (view.getOnItemLongClickListener() != null) {
                    handled = view.getOnItemLongClickListener().onItemLongClick(view, CHZZSwipeItemLayout.this, p, vId);
                }
                if (handled) {
    ...
    private void slideTo(int isOpen) {
        if (isOpen == 1) {
            ...
            if (mDelegate != null) {
                mDelegate.onCHZZSwipeItemLayoutOpened(this);
            }
        } else {
            mBottomView.setVisibility(INVISIBLE);
            mCurrentStatus = Status.Closed;
            if (mDelegate != null) {
                mDelegate.onCHZZSwipeItemLayoutClosed(this);
            }
        }
    ...
    private ViewDragHelper.Callback mDragHelperCallback = new ViewDragHelper.Callback() {
        ...
        public void onViewReleased(View releasedChild, float xvel, float yvel) {
            ...
            ViewCompat.postInvalidateOnAnimation(CHZZSwipeItemLayout.this);
        }
    ...
    private void dispatchSwipeEvent() {
        ...
        if (mCurrentStatus != preStatus) {
            if (mCurrentStatus == Status.Closed) {
                ...
                mBottomView.setVisibility(VISIBLE);
                if (mDelegate != null) {
                    mDelegate.onCHZZSwipeItemLayoutStartOpen(this);
                }
            }
        }
    }
    ...
    public interface CHZZSwipeItemLayoutDelegate {
        /**
         * 变为打开状态
         *
         * @param swipeItemLayout
         */
        void onCHZZSwipeItemLayoutOpened(CHZZSwipeItemLayout swipeItemLayout);
         ...
         * @param swipeItemLayout
         */
        void onCHZZSwipeItemLayoutClosed(CHZZSwipeItemLayout swipeItemLayout);
         ...
         * @param swipeItemLayout
         */
        void onCHZZSwipeItemLayoutStartOpen(CHZZSwipeItemLayout swipeItemLayout);
    }
