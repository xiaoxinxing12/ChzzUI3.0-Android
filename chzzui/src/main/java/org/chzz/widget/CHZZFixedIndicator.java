...
public class CHZZFixedIndicator extends LinearLayout implements View.OnClickListener, View.OnFocusChangeListener {
    private static final String TAG = CHZZFixedIndicator.class.getSimpleName();
    ...
    public CHZZFixedIndicator(Context context) {
        this(context, null);
    }

    public CHZZFixedIndicator(Context context, AttributeSet attrs) {
        super(context, attrs);
    ...
    private void initCustomAttrs(Context context, AttributeSet attrs) {
        TypedArray typedArray = context.obtainStyledAttributes(attrs, R.styleable.CHZZIndicator);
    ...
    public void initCustomAttr(int attr, TypedArray typedArray) {
        if (attr == R.styleable.CHZZIndicator_indicator_triangleColor) {
            mTriangleColor = typedArray.getColor(attr, mTriangleColor);
        } else if (attr == R.styleable.CHZZIndicator_indicator_triangleHorizontalMargin) {
            /**
        ...
        } else if (attr == R.styleable.CHZZIndicator_indicator_triangleHeight) {
            mTriangleHeight = typedArray.getDimensionPixelSize(attr, mTriangleHeight);
        } else if (attr == R.styleable.CHZZIndicator_indicator_textColor) {
            mTextColor = typedArray.getColorStateList(attr);
        } else if (attr == R.styleable.CHZZIndicator_indicator_textSizeNormal) {
            mTextSizeNormal = typedArray.getDimensionPixelSize(attr, mTextSizeNormal);
        } else if (attr == R.styleable.CHZZIndicator_indicator_textSizeSelected) {
            mTextSizeSelected = typedArray.getDimensionPixelSize(attr, mTextSizeSelected);
        } else if (attr == R.styleable.CHZZIndicator_indicator_hasDivider) {
            mHasDivider = typedArray.getBoolean(attr, mHasDivider);
        } else if (attr == R.styleable.CHZZIndicator_indicator_dividerColor) {
            mDividerColor = typedArray.getColor(attr, mDividerColor);
        } else if (attr == R.styleable.CHZZIndicator_indicator_dividerWidth) {
            mDividerWidth = typedArray.getDimensionPixelSize(attr, mDividerWidth);
        } else if (attr == R.styleable.CHZZIndicator_indicator_dividerVerticalMargin) {
