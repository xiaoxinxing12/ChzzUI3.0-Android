 ...
 * @param <M> 适配的数据类型
 */
public class CHZZRecyclerViewAdapter<M> extends RecyclerView.Adapter<CHZZRecyclerViewHolder> {
    ...
    @Override
    public CHZZRecyclerViewHolder onCreateViewHolder(ViewGroup parent, int viewType) {
        CHZZRecyclerViewHolder viewHolder = null;
        if (itemLayout != null && itemLayout.length > 0) {
            for (int a : itemLayout) {
                if (viewType == a) {
                    viewHolder = new CHZZRecyclerViewHolder(mRecyclerView, LayoutInflater.from(mContext).inflate(a, parent, false), false, (mHeadViews != null ? true : false), mOnRVItemClickListener, mOnRVItemLongClickListener);
        ...
        if (viewType == -999) {
            viewHolder = new CHZZRecyclerViewHolder(mRecyclerView, mHeadViews, true, true, mOnRVItemClickListener, mOnRVItemLongClickListener);
        } else if (viewType == -998) {
            viewHolder = new CHZZRecyclerViewHolder(mRecyclerView, mFootViews, true, true, mOnRVItemClickListener, mOnRVItemLongClickListener);
        } else if (itemLayout == null || itemLayout.length == 0) {
            viewHolder = new CHZZRecyclerViewHolder(mRecyclerView, LayoutInflater.from(mContext).inflate(mItemLayoutId, parent, false), false, (mHeadViews != null ? true : false), mOnRVItemClickListener, mOnRVItemLongClickListener);
            viewHolder.getViewHolderHelper().setOnItemChildClickListener(mOnItemChildClickListener);
            viewHolder.getViewHolderHelper().setOnItemChildLongClickListener(mOnItemChildLongClickListener);
            viewHolder.getViewHolderHelper().setOnItemChildCheckedChangeListener(mOnItemChildCheckedChangeListener);
            setItemChildListener(viewHolder.getViewHolderHelper());
    ...
    protected void setItemChildListener(CHZZViewHolderHelper helper) {
        if (null != mItemChildListener)
            mItemChildListener.setItemChildListener(helper);
    }

    @Override
    public void onBindViewHolder(CHZZRecyclerViewHolder viewHolder, int position) {
    ...
    public int getItemViewType(int position) {
        if (mHeadViews != null && position == 0) {
            return -999;
        }
        if (mFootViews != null && (((mHeadViews != null && position == getItemCount())) || position == getItemCount() - 1)) {
            return -998;
        }
        return position;
    }
    ...
    protected void fillData(CHZZViewHolderHelper helper, int position, M model) {
        if (iFillDataListener != null)
            iFillDataListener.setFillDataListener(helper, position, model);
    }
     ...
     */
    public void setOnRVItemClickListener(CHZZOnRVItemClickListener onRVItemClickListener) {
     ...
     */
    public void setOnRVItemLongClickListener(CHZZOnRVItemLongClickListener onRVItemLongClickListener) {
    ...
    public interface IFillDataListener<M> {
        /**
         * 给item设置数据
         *
